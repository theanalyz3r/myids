function getFileManagerWidthAndHeight(id, value) {
	var inputRightMargin = 0;
	var inputTopMargin = 0;
	var inputBottomMargin = 0;
	var inputBorder = 1;
	var addMargin = 0;
	var browseObj = document.getElementById('htmleditor_browse_' + id);
	var theInput = browseObj.parentNode.getElementsByTagName('input')[0];
	...
	if(value.indexOf('.jpg') < 0 && value.indexOf('.jpeg') < 0 && value.indexOf('.png') < 0 && value.indexOf('.gif') < 0) {
		newImg.src = ((settings.dds && settings.dds != "")?settings.dds:"") + 'webassist/kfm/themes/webassist_v2/icons/select_none.png';
		width = 1;
		height = 1;
		return;
	}
		...
		if (height !=0 && width!=0)  {
			...
			setTimeout("getFileManagerWidthAndHeight('"+id+"','"+value+"')",100);
		}
	}
...
function init_fileManager(){
	...
	for(i in els){
		...
		if(el.id && reg.test(el.id)) {
			...
			el.onclick=function(){
				...
				window.SetUrl=(function(id){
					return function(value){
						...
						getFileManagerWidthAndHeight(instanceID, value);
			...
			getFileManagerWidthAndHeight(regMatch[1], theInput.value);
		}
	}
}

if (window.addEventListener) {
  window.addEventListener('DOMContentLoaded', init_fileManager, false);
}
